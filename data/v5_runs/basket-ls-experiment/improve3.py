"""Robust validation: does Sharpe-weighting survive WALK-FORWARD (weights from PAST
data only, no lookahead)? Also test a simpler robust prior: drop weak eq classes."""
import sys, os
...
CL=pd.DataFrame(csd).dropna()

def eq(): return z(sum(z(CL[c]) for c in CL.columns))
def static_sw():   # in-sample (lookahead) — for reference
    w={c:max(0.2,sh(CL[c])) for c in cls}; return z(sum(w[c]*z(CL[c]) for c in cls))
def walkfwd_sw(cap=None):   # weights from EXPANDING trailing Sharpe (no lookahead), shifted
    ann=np.sqrt(252); parts=[]
    for c in cls:
        r=CL[c]
        roll_sh=(r.expanding(min_periods=252).mean()/r.expanding(min_periods=252).std()*ann).shift(1)
        w=roll_sh.clip(lower=0.2).fillna(0.2)
        if cap: w=w.clip(upper=cap)
        parts.append(w*z(r))
    port=sum(parts)/len(cls); return z(port)

variants={"equal-class (current)":eq(), "sharpe-wt STATIC (lookahead)":static_sw(),
          "sharpe-wt WALK-FWD":walkfwd_sw(), "sharpe-wt WALK-FWD cap2":walkfwd_sw(cap=2.0)}
print(f"{'variant':30} {'evalSR':>7} {'2021+':>7} {'maxDD':>7} | FP pass@7%")
for nm,b in variants.items():
    e=(1+b.loc['2017-01-01':]).cumprod(); dd=float((e/e.cummax()-1).min()*100)
    s=fp_sim(b.dropna().values,0.7,day_safety=1.5,p1=0.08,p2=0.05,dayloss=0.05,maxloss=0.10)
    print(f"{nm:30} {sh(b):+7.3f} {sh(b,'2021-01-01'):+7.2f} {dd:6.1f}% | pass {s['passpct']:.1f}% failDD {s['fail_dd']:.1f}% med {s['med_mo']:.0f}mo")
