"""
backtest_option_a_expanding_wf.py — Option A: Expanding-window WF, per-fold fresh encoder.

Unlike the sliding-120d clean WF (backtest_candle_clean_wf.py) which starves the encoder
of data on every fold, this script uses an EXPANDING window:
  - Fold 1: encoder + CatBoost trained on 180d (~6k bars)
  - Fold 2: encoder + CatBoost trained on 240d (~8k bars)
  - ...
  - Fold N: encoder + CatBoost trained on 180 + (N-1)*60 days

Later folds accumulate more encoder training data, giving the encoder time to converge.
No leakage: encoder never sees each fold's OOS test period.

Walk-forward:
  - Expanding window: 180d min train, step=60d
  - CatBoost: 300 iters, depth=6, lr=0.05, l2=3.0, subsample=0.8
  - Threshold: 0.60, SL=10p, TP=30p, spread=1.0p, comm=0.5p

Usage:
    conda run -n envmt5 python scripts/backtest_option_a_expanding_wf.py
    conda run -n envmt5 python scripts/backtest_option_a_expanding_wf.py --symbol EURUSD
"""
...
def _get_expanding_folds(index: pd.DatetimeIndex, min_train_days: int,
                         step_days: int, test_days: int):
    """Generate (fold_idx, train_start, train_end, test_end) for expanding WF."""
    start = index[0]
    end   = index[-1]
    td_min = pd.Timedelta(days=min_train_days)
    te     = pd.Timedelta(days=test_days)
    step   = pd.Timedelta(days=step_days)

    folds = []
    fold_idx  = 0
    train_end = start + td_min
...
def run_option_a(symbol: str) -> None:
    cfg_s    = SYMBOL_CFG[symbol]
    pip_size = cfg_s["pip_size"]

    print(f"\n{'='*72}")
    print(f"  OPTION A — EXPANDING WF, PER-FOLD FRESH ENCODER — {symbol}")
    print(f"  Min train: {MIN_TRAIN_DAYS}d  Step: {STEP_DAYS}d  Test: {TEST_DAYS}d")
    print(f"  Encoder grows each fold — later folds have more encoder data")
    print(f"{'='*72}\n")
    ...
    folds = _get_expanding_folds(df_raw.index, MIN_TRAIN_DAYS, STEP_DAYS, TEST_DAYS)
    print(f"  Folds: {len(folds)}  (expanding {MIN_TRAIN_DAYS}d min, step={STEP_DAYS}d)\n")

    header = (f"  {'Fold':>4}  {'Train window':>25}  {'Test window':>25}  "
              f"{'TrBars':>7}  {'Trd':>4}  {'Win%':>6}  {'Sharpe':>7}")
    print(header)
    print("  " + "-" * (len(header) - 2))
    ...
    for fold_idx, train_start, train_end, test_end in folds:
        ...
        print(f"  {fold_idx:>4}  "
              ...
              f"{len(df_train):>7}  {n_t:>4}  {win_r:>5.0%}  {sh_s:>7}")

        all_trades.extend(fold_trades)

    print(f"\n  Elapsed: {(time.time()-t0)/60:.1f} min")
    print(f"\n{'='*72}")
    print(f"  FINAL RESULT — {symbol} (OPTION A: expanding WF, per-fold encoder)")
    print(f"{'='*72}")
    ...
    if symbol in orig:
        ...
        print(f"    Option A expanding WF (this script) : {overall_sh:+.3f}")
    print()
...
def main():
    ...
    print(f"\n{'='*72}")
    print(f"  OPTION A — EXPANDING WINDOW WF WITH PER-FOLD FRESH ENCODER")
    print(f"  Encoder grows each fold — tests if accumulating data fixes encoder quality")
    print(f"{'='*72}")
